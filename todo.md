...
Five passes over every source file in the repository (applet, libraries, helper
script, packaging, docs) have been worked through and closed: defects and gaps,
then architecture and coupling, then the interface against
[lawsofux.com](https://lawsofux.com/), then bugs and incongruences, then cost,
timing, teardown and state. Build output, caches and `.git` were skipped; the
project has no generated files.

The conventions the list used, for whoever adds to it next:
