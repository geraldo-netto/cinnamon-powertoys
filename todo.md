...
| PT-145c | medium | S | Whole probes, one at a time — a prerequisite for the above rather than a nicety. [`_detect`](cinnamon-powertoys@geraldo-netto/lib/ddc.js#L545) lowers `_detecting` the moment the detect answers, while the `getvcp` calls that same answer has just started are still out. That is harmless when the only thing asking is a monitor being plugged in, which the desktop says exactly once; at one probe a second it means the next tick sends a detect across every bus while the last probe is still reading one of them, and two ddcutil talking to one monitor is how ddcutil comes back with nothing — PT-135 met from the other end. `_detecting` has to cover the reads too, and a tick that lands inside a probe should be dropped rather than queued: the next one is a second away, and a queue of probes against hardware that answers in tenths of a second never empties. |
