...
| PT-117 | low | XS | Dead declarations, and one field nothing reads. `applet.js` still opens with [`const Util`](cinnamon-powertoys@geraldo-netto/applet.js#L23) and [`const UPDeviceLevel`](cinnamon-powertoys@geraldo-netto/applet.js#L63); neither is referenced anywhere in the file, `Util` having gone unused when the *Configure Power Toys* row was removed. [`lib/upower.js`](cinnamon-powertoys@geraldo-netto/lib/upower.js#L128) sets `charging:` on every battery power reading and nothing anywhere reads it. |
