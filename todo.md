...
| id | severity | effort | description |
|----|----------|--------|-------------|
| PT-74 | high | M | **The poll reads sensors nobody asked to see, and those are the slow ones.** Measured on this machine: an update costs 10.0 ms, of which `SensorSet.read()` is 9.0 ms. The most expensive individual reads are the disk sensors — `nvme Composite` 1.6 ms, `drivetemp (sdb)` 1.4 ms, `drivetemp (sdd)` 1.2 ms — because reading them wakes the drive. Those are exactly the sensors the default *Include disk, network and board sensors* setting hides. Read what will be displayed and the default poll drops to about a millisecond. |
...
| PT-80 | low | XS | New hardware is only noticed when the menu is opened. `SensorSet.refresh()` is called from `_onMenuOpened()` and nowhere else, so since PT-18 a card that wakes up while the menu is shut never reaches the panel. |
