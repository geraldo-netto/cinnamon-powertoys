...
| PT-54 | medium | S | Every device and battery icon is an `xsi-` name from `xapp-symbolic-icons` ([lib/format.js](cinnamon-powertoys@geraldo-netto/lib/format.js)). Cinnamon's own power applet only started using that set in 6.6, so on the 5.4 to 6.4 desktops the version claim covers, the package is probably absent and those icons are blank. Found while settling PT-24, which is why the claim now names the package. Fall back to the plain `battery-*` and `input-*` names when the `xsi-` one is not in the icon theme. |
...
| PT-50 | low | XS | The charge limit is read out of the sysfs node by `_collect()` rather than by the object that discovered it, whose own `value` field is captured once at startup and never refreshed. Give the charge control a live getter, and the reading stops being the only field the applet fetches by path. |
