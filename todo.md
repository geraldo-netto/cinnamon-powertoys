...
|----|----------|--------|-------------|
| PT-82 | high | S | **One thrown error stops the applet for the rest of the session.** `_update()` sets `_collecting` and only clears it inside the callback `_collect()` makes on success; `_collect()` returns without calling back when `_assemble()` throws, and `_update()` catches nothing if `readAsync` throws before it ever gets that far. Verified on the running applet: one exception inside the assembly leaves `_collecting` true, and every later poll — timer, menu open, settings change — returns at the guard. The panel and menu keep the last reading and nothing says why; only reloading the applet recovers. A proxy disappearing between two polls is enough to trigger it. |
...
| PT-85 | medium | S | `CpuControl.snapshot()` is the blocking IO left in a poll. Four synchronous reads on the compositor's main loop every four seconds, one of them `cpuinfo_max_freq`, which cannot change while the machine is running and is not cached. Since PT-75 moved the sensors off, this and PT-84 are the only file reads a poll still makes where it draws. |
