...
| PT-126 | low | XS | When bluetoothd vanishes, [`BluezBatteries._refresh`](cinnamon-powertoys@geraldo-netto/lib/bluez.js#L181) clears `devices` and returns without calling `_onChanged`, though clearing a non-empty list is exactly the kind of change the callback exists for. The menu keeps the dead rows until the next poll sweeps them — up to the refresh interval. Track whether anything was dropped and say so, the same way the populated path already does. |
